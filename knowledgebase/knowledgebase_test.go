...
func TestNewKnowledgeBase_WithStringBackendAndValidConfig(t *testing.T) {
	...
	mockey.PatchConvey("redis backend with valid config via mock constructor", t, func() {
		var m _interface.KnowledgeBackend = &mockBackend{}
		mockey.Mock(redis_knowledge_backend.NewRedisKnowledgeBackend).Return(m, nil).Build()

		kb, err := NewKnowledgeBase(
			ktypes.RedisBackend,
			WithBackendConfig(&redis_knowledge_backend.Config{Index: "idx"}),
			WithName("n"),
			WithDescription("d"),
		)
		assert.Nil(t, err)
...
func TestNewKnowledgeBase_InvalidConfigType(t *testing.T) {
	...
	mockey.PatchConvey("redis backend with invalid config type", t, func() {
		kb, err := NewKnowledgeBase(
			ktypes.RedisBackend,
			WithBackendConfig(struct{}{}),
		)
